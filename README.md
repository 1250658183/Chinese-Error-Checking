...
```
基于BERT模型的纠错效果（不进行检错），更新了其漏字处理的模块，目前尚存在一些问题，但目前展示的漏字准召率高于ngram模型。
...
<table border="1">
	<tbody>
		...
		<tr>
			...
			<td rowspan="3">
				<p>
					将BERT模型与ngram模型尝试进行了融合，用来检测漏字处理的性能
				</p>
			</td>
			<td>
				<p>
			...
			<td rowspan="3">
				<p>
					目前两个模型的组合，在加字后新句子的打分尚存在一些问题，但是目前评测的只针对漏字的处理的纠错准召率要高于ngram模型
				</p>
			</td>
		</tr>
		<tr>
