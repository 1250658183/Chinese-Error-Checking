...
- 目前已有优化思路的方向包括：针对‘ta'等混用的beam_search算法的引入；
- 针对词法相关’de'错误的词法分析的引入，如在检错模块或设置规则过滤；
- 针对连续性出错的递归预测的测试实验表现较差，可能需要对其进行进一步探索；
- 继续完成SpellGCN开源代码中的一些TODO功能项，原始的基于TF改起来比较慢，本周想看看赵飞师兄有没有时间帮忙看几个BUG，同时也在考虑用pytorch重新实现一份进行测试。

- [ ] 本周优先实现界面的开发，周二调研了部分数据库及表单交互的相关逻辑设计，周三调研用户部分的相关逻辑设计，完成之后可以进一步完善界面静态元素部分。
- [ ] 本周将优先选择一些编程量小的模型优化方向进行尝试，如beam_search算法的引入，或者已经完成部分代码编制的递归预测的测试之上。
- [ ] 针对词法规则的引入以及SpellGCN开源代码的复现可能要在下周才能抽取多数精力进行。
