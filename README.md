...
| 更新文件                    | 算法简介                                                     | 简易对比                                                     | 简易分析                                                     |
| --------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| origin0228.txt ours0228.txt | 在当前版本中，已经划分出测试集、整合抽取出的领域专有名词、使用贪心策略在每个纠错步进行最优词的纠错，检错Threshold=1（越小发现的可能错词越多）、限定词语的候选词为相同简拼。 | 原始模型:        same_pinyin: 59/98;   same_stroke: 30/52;   miss_char: 0/100;  change_word: 32/100;    我们的模型：  same_pinyin: 40/98; same_stroke: 20/52; miss_char: 16/100; change_word: 19/100; | 原始模型在添加候选词时存在与原始词拼音相同的限制，我们尝试因此加入简拼相同的限制，用以较少候选词的发散性，实验效果有较刚划分测试集时有部分提升，但仍弱于原始模型结果。认为原因是语料模型过小导致。 |
|                             |                                                              |                                                              |                                                              |
